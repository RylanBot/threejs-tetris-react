import { Block, Tetriminos, type TetroType } from "@/components/Tetrimino";
...
export const getRandomTetrimino = (() => {
    ...
    const shuffleArray = (array: TetroType[]): TetroType[] => {
    ...
    const fillBag = () => {
        const tetriminos = Object.keys(Tetriminos) as TetroType[];
        for (let i = 0; i < 3; i++) {
            bag.push(...tetriminos);
        }
        shuffleArray(bag);
 ...
 * 随机获取下落位置
 */
export function getRandomPosition(rotatedBlocks: Block[]): ThreePosition {
